  ...
  return (
    <DashboardLayout roleTitle="Alumno" userName="Alumno" userRole="Estudiante" menuGroups={menuGroups} breadcrumbs={['Alumno', 'Conducta']}>
      <div className="flex items-center gap-4 mb-8">
        ...
        {!loading && (
          <span className={`px-3 py-1 rounded-full text-sm font-bold ${totalPoints < 0 ? 'bg-danger/15 text-danger' : 'bg-success/15 text-success'}`}>
              ...
              <div className="flex items-center gap-2 mb-1">
                <span
                  className={`px-2 py-0.5 rounded text-xs font-semibold ${
                    r.type === 'merit' ? 'bg-success/15 text-success' : r.type === 'demerit' ? 'bg-danger/15 text-danger' : 'bg-gray-100 text-gray-700'
                  }`}
                >
                  {r.type === 'merit' ? 'Mérito' : r.type === 'demerit' ? 'Demérito' : 'Incidente'}
                </span>
                {r.category && <span className="text-xs text-gray-500">{r.category}</span>}
                <span className={`text-xs font-bold ${r.points < 0 ? 'text-danger' : r.points > 0 ? 'text-success' : 'text-gray-500'}`}>
