...
export default async function GradebookPage({ searchParams }: { searchParams: Promise<{ class?: string, term?: string }> }) {
  ...
  if (!defaultTermId && options.terms.length > 0) {
    defaultTermId = options.terms[0].id; // Fallback to first available term
  }
