...
import { Button, Card, DatePicker, Input, message as antdMessage } from 'antd';
...
export default function TeacherCalendarPage() {
  ...
  return (
    <DashboardLayout roleTitle="Docente" userName="Docente" userRole="Docente" menuGroups={menuGroups} breadcrumbs={['Docente', 'Calendario']}>
      <div className="flex items-start justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Calendario Escolar</h1>
          <div className="text-sm text-gray-500">Vista de eventos globales del ciclo.</div>
        </div>
        ...
        <Card className="lg:col-span-2" title={`Eventos (${filtered.length})`}>
          <div className="space-y-0">
            {loading ? (
              ...
              <div className="py-3 text-sm text-gray-500">No hay eventos.</div>
            ) : (
              filtered.map((item) => (
