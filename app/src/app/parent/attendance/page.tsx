...
import { getParentDashboardData, getChildAttendanceSummary } from '@/actions/parent';
...
export default function ParentAttendancePage() {
  ...
  const loadAttendance = async (studentId: string) => {
    ...
    setLoading(true);
    try {
      const summary = await getChildAttendanceSummary(studentId);
      ...
      message.error(error.message || 'Error al cargar asistencia');
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    (async () => {
      ...
      if (firstChild) await loadAttendance(firstChild);
  ...
  return (
    <DashboardLayout roleTitle="Portal de Padres" userName="Familia" userRole="Padre/Tutor" menuGroups={menuGroups} breadcrumbs={['Familia', 'Asistencia']}>
