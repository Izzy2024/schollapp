...
async function seedCafeteriaManage(tenantId: string, userId: string) {
  const permission = await prisma.permission.upsert({
    where: { code: 'cafeteria:manage' },
    update: {},
    create: { code: 'cafeteria:manage', description: 'test seed: cafeteria manage access' },
  });
  const role = await prisma.role.create({ data: { tenantId, name: `cafeteria-manager-${userId}` } });
...
async function makeTenantWithAdmin(slugPrefix: string) {
  ...
  await seedCafeteriaManage(tenant.id, admin.id);
  return { tenant, admin };
}
