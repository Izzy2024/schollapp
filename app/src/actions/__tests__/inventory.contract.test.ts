...
async function seedInventoryManage(tenantId: string, userId: string) {
  const permission = await prisma.permission.upsert({
    where: { code: 'inventory:manage' },
    update: {},
    create: { code: 'inventory:manage', description: 'test seed: inventory manage access' },
  });
  const role = await prisma.role.create({ data: { tenantId, name: `inventory-manager-${userId}` } });
...
async function makeTenantWithAdmin(slugPrefix: string) {
  ...
  await seedInventoryManage(tenant.id, admin.id);
  return { tenant, admin };
}
